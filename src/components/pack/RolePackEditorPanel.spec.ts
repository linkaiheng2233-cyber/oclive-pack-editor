...
describe('RolePackEditorPanel', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('loads pack after picking directory', async () => {
    ...
    await flushPromises()
    expect(api.invokeLoadRolePackForEditor).toHaveBeenCalledWith('C:\\demo\\roles\\demo.pack')
    expect(w.text()).toContain('demo.pack')
  })

  it('switches to JSON mode', async () => {
    vi.mocked(open).mockResolvedValueOnce('C:\\rp')
    vi.mocked(api.invokeLoadRolePackForEditor).mockResolvedValueOnce({
      manifestText: JSON.stringify(manifestMinimal, null, 2),
      settingsText: JSON.stringify(settingsMinimal, null, 2),
      mergedSceneIds: ['default'],
