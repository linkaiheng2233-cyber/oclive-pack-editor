 ...
 *
 * - OCLIVE_VALIDATION_CRATE：指向 `oclive_validation` 的 Cargo 目录（含 Cargo.toml）
...
const defaultCrate = join(editorRoot, '..', 'oclivenewnew', 'crates', 'oclive_validation')
const defaultOut = join(editorRoot, 'src', 'wasm', 'pkg')

const crate = process.env.OCLIVE_VALIDATION_CRATE ?? defaultCrate
const outDir = process.env.OCLIVE_WASM_OUT ?? defaultOut

if (!existsSync(join(crate, 'Cargo.toml'))) {
  console.error(
    `[wasm-pack-build] 找不到 crate：${crate}\n` +
      '请设置 OCLIVE_VALIDATION_CRATE 指向 oclive_validation 目录，或将 oclivenewnew 放在与本编写器同级目录。',
  )
  process.exit(1)
}
...
const r = spawnSync(
  'wasm-pack',
  ['build', crate, '--target', 'web', '--out-dir', outDir, '--features', 'wasm'],
