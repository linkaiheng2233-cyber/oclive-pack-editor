...
5. **保存**：校验通过后可写回 **`pipeline.ocblueprint`**（及 `prompts/reply_quality_anchor.md` 等附属文件）；不保留 legacy 三件套。保存时**保留**旧蓝图中主应用写入的 **`includes`、`groups`、`expert_overlay`、`runtime_config`**，仅更新 `meta` / `slot_registry`。
...
- **编写器不负责**生成或校验 `expert_routing.json`。高级创作中的 **`expert/default.oclexpert`** 为**已废弃实验格式**，界面已提示勿新建；与专家路由无关。
